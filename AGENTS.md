...
- 커밋 메시지는 저장소의 기존 이력과 같은 `type: 한국어 변경 요약` 형식을 사용한다.
  - `feat`: 사용자 기능 추가·연결
  - `fix`: 사용자 동작·UI·데이터 오류 수정
  - `test`: 자동 검사·smoke·회귀 검증 추가 또는 강화
  - `docs`: 문서·audit·검증 결과·로드맵 갱신
  - `chore`: 브랜드·빌드·배포·개발 도구처럼 제품 기능 외 작업
- 요약은 짧은 명사형으로 작성하고 마침표를 붙이지 않는다. 예: `feat: Task 알림 시각 설정 연결`, `fix: Workspace Task 메뉴 초점 이동 개선`, `docs: Android preview 빌드 상태 갱신`.
