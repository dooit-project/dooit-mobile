...
- [x] 게스트 access token 만료 전에 같은 guest user id를 유지하는 `POST /api/v1/auth/guest/refresh` 계약과 모바일 API를 반영한다. 갱신 실패 시 기존 token을 유지하며, 만료 시 새 게스트를 자동 생성해 기존 데이터 연결을 끊지 않는다.
