...
- [ ] 게스트 생성 또는 병합 실패 시 기존 게스트 token과 데이터를 유지하고 사용자가 재시도할 수 있는 오류 UI를 제공한다. 게스트 생성과 저장 token 확인 실패는 데이터 화면을 열지 않고 앱 시작 단계에서 재시도하도록 구현했으며, 병합 실패 처리는 백엔드 계약 이후 추가한다.
