...
- [ ] 게스트 상태의 신규 회원가입은 같은 user id를 정식 계정으로 승격하고, 기존 이메일 계정과의 충돌 및 실패 rollback 정책을 확정한다. 모바일과 mock API는 승격 `TokenResponse`를 받으면 추가 로그인 없이 정식 token·cache로 전환하고 연결 완료 안내를 표시하도록 준비했다. 실제 rollback은 백엔드 검증 후 완료한다.
...
- [ ] 로그인·회원가입 성공 후 정식 token으로 교체하기 전에 서버 병합 성공을 확인하고, 성공 시 사용자 범위 Query cache를 초기화해 다시 조회한다. 모바일 로그인은 성공 응답 이후에만 정식 token과 인증 cache를 교체하며, 실패 시 기존 게스트 token을 유지하도록 테스트를 추가했다. 회원가입 승격은 백엔드 계약 이후 반영한다.
- [ ] 게스트 생성 또는 병합 실패 시 기존 게스트 token과 데이터를 유지하고 사용자가 재시도할 수 있는 오류 UI를 제공한다. 게스트 생성과 저장 token 확인 실패는 데이터 화면을 열지 않고 앱 시작 단계에서 재시도한다. 로그인 자격 증명 실패 `401/11001`은 공통 세션 만료와 분리해 게스트 token을 유지하며, 실제 병합 rollback smoke는 backend 배포 후 확인한다.
...
- [ ] 게스트 생성·병합 real API smoke test를 추가하고 데이터 소유권, 관계 무결성, 중복 미발생 결과를 [`SMOKE_TEST_LOG.md`](../qa/SMOKE_TEST_LOG.md)에 기록한다. `npm run smoke:guest:real`은 token을 출력하지 않고 게스트 발급·갱신, Task 소유권 병합, 후속 로그인 중복 미발생을 검증한다. 일정·반복·D-Day 관계 검증은 남아 있다.
...
- [ ] 저장 token 확인 실패, 정식 token 만료, 게스트 token 만료를 서로 다른 복구 상태로 표시하고 새 guest id 자동 발급을 막는다.
...
- [ ] 계정 연결 성공 결과를 데이터 종류별 개수로 표시할지 백엔드 응답 계약을 확인하고, 미지원이면 일반 성공 문구를 유지한다.
- [ ] 최초 설치, 앱 재실행, 게스트 발급 실패, offline, API 4xx/5xx, 로그인 우회, 가이드 재실행을 Android, iOS, Web에서 검증한다.
- [ ] 320px·390px·430dp, 큰 글꼴, 키보드, VoiceOver·TalkBack에서 시작 선택과 오류 복구가 막히지 않는지 확인한다.
