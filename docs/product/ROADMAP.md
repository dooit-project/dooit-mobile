...
- [ ] 저장 token 확인 실패, 정식 token 만료, 게스트 token 만료를 서로 다른 복구 상태로 표시하고 새 guest id 자동 발급을 막는다. 실행 중 만료는 cache의 account type을 기준으로 정식 회원 재로그인과 게스트 데이터 보호 안내를 구분한다. cold start에서 만료된 token의 계정 유형 복원은 남아 있다.
