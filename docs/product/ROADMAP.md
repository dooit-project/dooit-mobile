...
- [ ] 게스트 생성 또는 병합 실패 시 기존 게스트 token과 데이터를 유지하고 사용자가 재시도할 수 있는 오류 UI를 제공한다. 게스트 생성과 저장 token 확인 실패는 데이터 화면을 열지 않고 앱 시작 단계에서 재시도한다. 로그인·회원가입 요청 실패 시 게스트 데이터 유지와 재시도 안내를 표시하고, 자격 증명 실패 `401/11001`은 공통 세션 만료와 분리해 guest token을 유지한다. 회원가입 이메일 충돌 rollback은 real API smoke로 확인했으며 강제 병합 실패 rollback 검증은 남아 있다.
