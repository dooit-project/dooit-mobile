...
#### B1. 비회원 게스트 계정과 정식 계정 연동

목표: 처음 앱을 연 사용자가 로그인 없이 바로 Task와 일정을 만들고, 이후 로그인하거나 회원가입해도 게스트 데이터를 잃지 않고 정식 계정에 연결한다.
...
- [ ] 게스트 생성 또는 병합 실패 시 기존 게스트 token과 데이터를 유지하고 사용자가 재시도할 수 있는 오류 UI를 제공한다. 게스트 생성과 저장 token 확인 실패는 데이터 화면을 열지 않고 앱 시작 단계에서 재시도한다. 로그인·회원가입 요청 실패 시 데이터 유지 안내와 `게스트로 계속 사용` 동선을 제공하고, 자격 증명 실패 `401/11001`은 공통 세션 만료와 분리해 guest token을 유지한다. 회원가입 이메일 충돌 rollback은 real API smoke로 확인했으며 강제 병합 실패 rollback 검증은 남아 있다.
- [x] 프로필에 `게스트로 사용 중`, 로그인 유도, 앱 삭제 시 복구 제한을 안내하고 게스트 로그인·회원가입 성공 후 Today에 데이터 연결 완료 결과를 보여준다. 실제 병합 정확성은 real API smoke에서 별도로 검증한다.
...
- [x] mock API에 게스트 발급, 게스트 `/auth/me`, 같은 user id를 유지하는 회원가입 승격, 기존 계정 로그인 후 mock 데이터 유지 동작을 추가한다. Web 새로고침에서도 저장된 mock guest token의 account type과 user id를 복원한다.
- [ ] 최초 실행, 앱 재실행, token 만료, 신규 회원가입 승격, 기존 계정 병합, 병합 재시도, 앱 삭제 후 재설치 시나리오를 Android, iOS, Web에서 검증한다. 단위 테스트에서는 저장된 게스트 token으로 같은 사용자를 복원하고, 저장 token 확인 실패 시 새 게스트를 만들지 않는 데이터 보존 기준을 확인한다.
- [x] 게스트 생성·병합 real API smoke test를 추가하고 데이터 소유권, 관계 무결성, 중복 미발생 결과를 [`SMOKE_TEST_LOG.md`](../qa/SMOKE_TEST_LOG.md)에 기록한다. `npm run smoke:guest:real`은 token을 출력하지 않고 게스트 발급·갱신, Task·일정·반복 series·D-Day 관계 병합, 후속 로그인 중복 미발생을 검증한다.

확정 제품 정책:

- 게스트는 로그인 회원과 동일하게 Task, 일정, 완료 기록, 반복 일정, D-Day, 검색을 사용할 수 있다. 이메일 인증, 비밀번호 변경, 여러 기기 동기화처럼 정식 계정이 필요한 기능만 제한한다.
- 현재 백엔드 계약의 게스트 token TTL은 31일이다. 같은 guest user id를 유지하는 갱신 계약이 준비되기 전에는 새 게스트 발급을 기존 계정 복구로 사용하지 않는다.
- 앱 삭제, 앱 데이터 초기화 또는 보안 저장소 유실 시 게스트 계정은 복구할 수 없다. 프로필에서 로그인 전 데이터의 복구 제한을 안내한다.
- 신규 회원가입은 게스트 user id를 유지한 채 `REGISTERED`로 승격한다.
- 기존 계정 로그인은 게스트의 모든 사용자 생성 데이터를 대상 계정에 추가하고, 제목이나 날짜가 같다는 이유로 자동 중복 제거하지 않는다.
- 기존 정식 계정의 계정 설정을 우선하며 게스트 설정은 정식 설정을 덮어쓰지 않는다. Task와 D-Day 관계, 반복 series와 occurrence, 완료 이력은 참조 무결성을 유지해 함께 이전한다.
- 병합은 정확히 한 번만 적용하며 실패 시 전체 rollback하고 기존 게스트 token과 데이터 접근을 유지한다.
- 정식 회원 로그아웃은 이전 게스트로 돌아가지 않고 새로운 게스트 계정을 시작한다.

완료 기준:

- 첫 설치 사용자는 별도 로그인 없이 서버 데이터 기능을 사용할 수 있다.
- 로그인 또는 회원가입 전 작성한 데이터가 정식 계정에 정확히 한 번 연결된다.
- 병합 실패나 네트워크 중단 시 게스트 인증과 데이터가 유실되지 않는다.
- 임의 guest id, IP 주소, 광고 ID, 하드웨어 식별자만으로 사용자 데이터에 접근할 수 없다.

#### B2. 최초 사용 온보딩과 오류 없는 진입

목표: 처음 앱을 연 사용자가 데이터 보존 방식을 이해하고 게스트 또는 정식 계정을 직접 선택하며, 인증·네트워크 오류가 Today의 데이터 영역에 노출되기 전에 복구할 수 있게 한다. 상세 제품 흐름과 문구는 [`FIRST_USE_FLOW.md`](../design/FIRST_USE_FLOW.md)를 기준으로 한다.
...
- [x] 인증 bootstrap을 `저장 세션 복원`과 `최초 사용자의 시작 방식 선택`으로 분리하고, token이 없을 때 게스트 API를 자동 호출하지 않는다. 저장 token 확인 실패도 새 게스트 생성으로 복구하지 않는다.
- [x] 최초 시작 화면에 `로그인 없이 시작`, `로그인 또는 계정 만들기`, 선택형 `기능 둘러보기`를 제공한다. 기능 둘러보기는 필요할 때만 펼쳐 Today, Calendar, D-Day의 역할을 짧게 설명한다.
- [x] 게스트 생성 실패 시 시작 화면에 머물면서 재시도와 로그인 우회를 모두 제공하고, 인증 준비 전에는 Today route 자체를 렌더링하지 않는 규칙을 테스트한다.
- [x] 저장 token 확인 실패, 정식 token 만료, 게스트 token 만료를 서로 다른 복구 상태로 표시하고 새 guest id 자동 발급을 막는다. token 응답의 account type을 Native SecureStore와 Web localStorage에 별도 보존해 cold start cache가 비어 있어도 정식 회원 재로그인과 게스트 데이터 보호 안내를 구분한다. 이 값은 화면 분기에만 사용하고 인증 권한은 token과 서버 응답으로만 판단한다.
- [x] 실제 신규 사용자에게 mock 샘플 데이터를 사용자 데이터처럼 표시하지 않고, 빈 Today에서 첫 Task를 바로 작성하는 성공 경험을 제공한다. mock 신규 사용자도 사용자별 빈 데이터로 시작하며 개발용 demo 계정만 기존 샘플을 유지한다.
- [x] Today의 빠른 기록·완료, Calendar의 일정, D-Day의 목표 연결을 실제 화면에서 한 번씩 안내하는 맥락형 도움말을 추가한다. 확인한 안내는 preference의 tip id로 저장해 다음 방문부터 숨긴다.
- [x] Settings에 `앱 사용 가이드 다시 보기`를 추가하고, 건너뛴 필수 온보딩을 다음 실행에 강제로 반복하지 않는다. 다시 보기는 `onboardingVersion`을 유지하고 화면별 tip 완료 상태만 초기화한다.
- [x] 알림 권한은 최초 실행에서 요청하지 않고 사용자가 알림이 필요한 일정이나 Task를 처음 저장할 때 이유를 설명한 뒤 요청한다. Android/iOS에서 미래 시각이 있는 활성 일정을 처음 저장한 경우에만 사전 안내를 표시하고, 동의하면 `expo-notifications` OS 권한을 요청한다. Web, Inbox, 완료·건너뜀·과거 일정과 안내 완료 사용자는 제외한다.
- [x] 계정 연결 성공 응답의 `mergeResult`로 Task, 일정, D-Day, 반복 series 개수를 Today에 표시한다. 회원가입 승격이나 구형 응답처럼 결과가 없으면 일반 성공 문구를 유지한다.
- [ ] 최초 설치, 앱 재실행, 게스트 발급 실패, offline, API 4xx/5xx, 로그인 우회, 가이드 재실행을 Android, iOS, Web에서 검증한다. Web 390×844 mock에서 최초 실행, 기능 둘러보기, 로그인 우회 후 `/start` 복귀, 게스트 빈 Today 진입과 인증 오류 미노출을 캡처했다.
- [ ] 320px·390px·430dp, 큰 글꼴, 키보드, VoiceOver·TalkBack에서 시작 선택과 오류 복구가 막히지 않는지 확인한다. Web 320×844, 390×844, 430×932에서 최초 실행과 기능 둘러보기의 반응형·스크롤을 캡처했고, 320px 로그인 선택에서 모든 입력과 복귀 행동이 DOM에 존재함을 확인했다. 네이티브 큰 글꼴·키보드·화면 읽기는 남아 있다.

완료 기준:

- 신규 사용자는 첫 데이터 query 전에 게스트 또는 정식 계정 시작 방식을 이해하고 선택한다.
- 게스트 발급이나 세션 복원이 실패해도 Today의 영역별 인증 오류를 보지 않고 재시도 또는 로그인으로 복구할 수 있다.
- 첫 Task 생성까지의 필수 흐름은 한 화면의 선택과 한 번의 입력으로 끝나며, 기능 설명은 건너뛰거나 나중에 다시 볼 수 있다.
- `로컬`, `게스트`, `계정 연결` 문구가 실제 데이터 저장·복구 계약과 모순되지 않는다.
...
| 보류 조건                                   | 연결 항목                                                                                  | 다음 행동                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ------------------------------------------- | ------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 게스트 API 포함 백엔드 재배포               | B1 게스트 발급, `/auth/me`, real API smoke                                                 | 2026-08-09 현재 백엔드 저장소에는 `POST /api/v1/auth/guest`가 있지만 local `127.0.0.1:8080` 실행본은 401을 반환한다. 게스트 security permit과 DB migration이 포함된 최신 backend commit/image를 배포한 뒤 `npm run smoke:guest:real`을 재실행한다.                                                                                                                                                                                                                           |
| 게스트 token 갱신 계약 확정                 | B1 token 만료, 앱 재실행, 데이터 보존                                                      | 같은 guest user id를 유지하는 갱신·재발급 endpoint와 만료 정책을 백엔드에서 확정한다. 새 `POST /auth/guest` 호출은 다른 user id를 만들므로 만료 복구에 사용하지 않는다. 계약 확정 후 모바일 token 교체와 만료 전 갱신 테스트를 추가한다.                                                                                                                                                                                                                                     |
| 게스트 승격·병합 백엔드 배포 및 검증 가능   | B1 회원가입 승격, 기존 계정 병합, rollback, 멱등성, 연결 완료 real smoke                   | 모바일과 mock은 승격 `TokenResponse`, 로그인 후 token/cache 교체, 실패 시 guest token 보존, 연결 완료 UI까지 준비했다. 백엔드 병합 transaction과 오류 계약이 배포되면 Task, 일정, 완료 이력, 반복 series/occurrence, D-Day 관계의 소유권 이전과 동시 재시도 중복 미발생을 real smoke로 검증한다.                                                                                                                                                                             |
...
| EAS Android signing credential 확인         | Android signing credential, preview APK build                                              | `npm run check:eas-setup`은 통과했고, 2026-08-08 재확인 기준 2026-08-03 preview build `684720b7-aa78-4a55-920d-d34995dd7a86`은 `FINISHED`다. 다만 해당 APK는 frontend commit `987c90d13216efddab3c70127f5d543704437bd8` 기준이라 이후 인증 UI 변경은 포함하지 않는다. 최신 UI APK가 필요하면 현재 `main` 기준으로 preview build를 다시 생성한다.                                                                                                                             |
...
1. 최신 `main` 기준 Android preview APK가 필요한지 결정한다. 기존 2026-08-03 APK는 생성 완료됐지만 이후 인증 UI 변경은 포함하지 않는다.
2. 최신 APK가 필요하면 Tailscale HTTPS URL이 들어간 preview APK를 다시 생성한다.
