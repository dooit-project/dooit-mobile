...
서버 focus 순서 복원·확정은 구현됐지만 사용자가 직접 선택·순서를 바꾸는 UI는 없다. 화면의 Today 순서 변경 안내는 실제 조작과 불일치하며 보완 대상이다. 서버 CLOSED 저장도 현재 마감 화면에서 호출하지 않는다. 이 둘을 구현 완료로 확대 해석하지 않는다. [검토 보고서](./PRODUCT_REVIEW_2026-09-14.md)를 따른다.
