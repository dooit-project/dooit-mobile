...
| 비밀번호 재설정          | `7f6feee`            | request·verify·confirm, TTL·rate limit·deep link 완료 | UI·deep link 연결 완료, 메일 smoke             |
...
| Workspace 초대 거절      | `144d18f`            | PENDING → REMOVED와 권한 테스트 완료                  | UI·cache mutation 완료, real smoke             |
| Task 알림 시각           | `bc5bd73`            | `notificationEnabled`·`notifyAt` 완료                 | 프론트 연결 완료, 실기기 smoke                 |

## production 반영 전 확인

2026-08-29 production 재검사에서 `GET /api/v1/system/metadata`가 HTTP 401을 반환했다. OpenAPI는 auth guest·login·refresh·guest refresh·logout과 비밀번호 재설정 request·verify·confirm 8개 POST의 성공(2xx) 응답, token response schema의 `refreshToken`·`refreshExpiresAt`가 여전히 누락됐다. metadata 익명 접근과 OpenAPI annotation을 보완한 배포 후 재검증한다.
...
1. 비밀번호 재설정 production 메일·deep link smoke
2. Workspace 초대 거절 real API smoke
