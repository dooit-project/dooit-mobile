...
| ------------------------------------------- | ------------------------------------------------------------------------- | ------------------ | ---------------------------------------------------- |
| `GET /api/v1/daily-plans/{date}/summary`    | 계획 확정 시점 focus 기준 완료·이동·미결정 집계                           | 미연결             | 타입·API·mock·query를 추가하고 하루 마감 결과에 연결 |
| `GET /api/v1/tasks/categories`              | 개인 Task만 집계하며 `category=null`은 `미분류`                           | 미연결             | 메뉴용 조회 계층과 loading·empty·error 상태 구현     |
| `/api/v1/tasks/{taskId}/checklist-items/**` | 개인·Workspace 지원. ACTIVE 멤버 조회, OWNER/EDITOR 변경, VIEWER 변경 403 | 미연결             | Task 상세의 한 단계 체크리스트와 권한별 UI 구현      |
| `POST /api/v1/tasks/quick-capture`          | `낼`, `내일모레/낼모레`, 상대 주+요일, `N시 반`, `HH:mm` 파싱             | real API 연결 완료 | mock parser·fixture·테스트를 서버 규칙과 맞춤        |

기존 개인 Task URL과 DTO에는 깨지는 변경이 없다. 카테고리 요약은 Workspace Task를 포함하지 않으며 카테고리 생성·이름 변경·삭제·사용자 지정 정렬 API를 대신하지 않는다.

## 프론트에서 바로 할 수 있는 일

1. mock 빠른 등록 parser에 신규 표현과 날짜 경계 테스트를 추가한다.
2. 세 신규 조회 영역의 타입, API client, mock fixture, query key와 오류 처리를 만든다.
3. 카테고리 요약을 좌측 메뉴에 연결하되 `전체`·`미분류`·개인 카테고리를 Workspace와 분리한다.
4. 개인·Workspace Task 상세에 체크리스트를 연결하고 VIEWER의 변경 행동을 숨기거나 비활성화한다.
5. 일일 계획 summary를 하루 마감 결과에 연결하고 확정 전·migration 미적용·404 상태를 구분한다.
6. mock 회귀 검사 후 local real API, production 배포 확인 뒤 Android 실기기 smoke를 수행한다.

UI가 바뀌는 3~5번은 Product Design 검토와 390×844 캡처 판정을 포함한다. 로컬 Web 검증은 ChatGPT 앱의 내장 브라우저를 기본으로 사용하고, 연결이 불가능하거나 Chrome 고유 동작을 확인해야 할 때 로컬 Chrome/Playwright를 사용한다.
